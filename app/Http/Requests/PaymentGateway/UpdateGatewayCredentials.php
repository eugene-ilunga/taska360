...
class UpdateGatewayCredentials extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if ($this->payment_method == 'freshpay' && $this->freshpay_status == 'active') {
            return $this->freshpayValidate();
        }
    ...
    private function freshpayValidate()
    {
        $rules['freshpay_mode'] = 'required|in:sandbox,live';

        if ($this->freshpay_mode == 'sandbox') {
            $rules['test_freshpay_merchant_id'] = 'required';
            $rules['test_freshpay_merchant_secret'] = 'required';
        }
        else {
            $rules['freshpay_merchant_id'] = 'required';
            $rules['freshpay_merchant_secret'] = 'required';
        }

        $rules['freshpay_encryption_key'] = 'nullable|min:16';
        $rules['freshpay_hmac_key'] = 'nullable|min:16';
        $rules['freshpay_firstname'] = 'nullable|string|max:191';
        $rules['freshpay_lastname'] = 'nullable|string|max:191';
        $rules['freshpay_email'] = 'nullable|email|max:191';
        $rules['freshpay_username'] = 'nullable|string|max:191';
