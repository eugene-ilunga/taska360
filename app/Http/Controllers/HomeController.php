...
class HomeController extends Controller
{
    ...
    public function invoice($hash)
    {
        ...
        $this->freshPayPendingPayment = Payment::where('invoice_id', $this->invoice->id)
            ->where('gateway', 'FreshPay')
        ...
        return view('invoice', [
            ...
            'freshPayPendingPayment' => $this->freshPayPendingPayment,
        ]);
    }
    ...
    public function freshpayModal(Request $request)
    {
        $this->id = $request->id;
        $this->type = $request->type;
        ...
        $this->company = $data->company;
        $this->amount = $request->type === 'order' ? $data->total : $data->amountDue();
        $this->currencyId = $data->currency_id;

        return view('public-payment.freshpay.index', $this->data);
    }
