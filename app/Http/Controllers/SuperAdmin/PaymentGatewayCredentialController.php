...
class PaymentGatewayCredentialController extends AccountBaseController
{
    ...
    public function index()
    {
        ...
        case 'square':
            ...
            break;
        case 'freshpay':
            $this->webhookRoute = route('billing.save_freshpay-webhook', [$hash]);
            $this->view = 'payment-gateway-settings.ajax.freshpay';
            break;
        case 'offline':
    ...
    public function update(UpdateGatewayCredentials $request, $id)
    {
        ...
        case 'square':
            ...
            break;
        case 'freshpay':
            $this->freshpay($request, $credential);
            break;
        default:
    ...
    private function freshpay($request, $credential)
    {
        $credential->freshpay_mode = $request->freshpay_mode;
        $credential->freshpay_merchant_id = $request->freshpay_merchant_id;
        $credential->freshpay_merchant_secret = $request->freshpay_merchant_secret;
        $credential->test_freshpay_merchant_id = $request->test_freshpay_merchant_id;
        $credential->test_freshpay_merchant_secret = $request->test_freshpay_merchant_secret;
        $credential->freshpay_encryption_key = $request->freshpay_encryption_key;
        $credential->freshpay_hmac_key = $request->freshpay_hmac_key;
        $credential->freshpay_firstname = $request->freshpay_firstname;
        $credential->freshpay_lastname = $request->freshpay_lastname;
        $credential->freshpay_email = $request->freshpay_email;
        $credential->freshpay_username = $request->freshpay_username;
        $credential->freshpay_status = $request->freshpay_status ? 'active' : 'deactive';
    }
