...
class BillingController extends AccountBaseController
{
    ...
    public function __construct()
    {
        ...
        if (in_array('active', [
            ...
            $this->stripeSettings->freshpay_status
        ])) {
    ...
    public function freshpayPayment(Request $request)
    {
        $request->validate([
            ...
            'phone' => [
                ...
                function ($attribute, $value, $fail) {
                    if (!app(FreshPayService::class)->isValidPhone($value)) {
        ...
        $credentials = GlobalPaymentGatewayCredentials::first();
        $freshPay = app(FreshPayService::class);

        if (!$credentials || $credentials->freshpay_status !== 'active') {
            return Reply::error(__('modules.freshpay.gatewayInactive'));
        }

        if (blank($freshPay->merchantId($credentials)) || blank($freshPay->merchantSecret($credentials))) {
        ...
        if ($plan->max_employees < $this->company->employees->count()) {
            return Reply::error('You can\'t downgrade package because your employees length is ' . $this->company->employees->count() . ' and package max employees count is ' . $plan->max_employees);
        }

        $amount = $plan->package == 'lifetime'
            ? $plan->price
            : ($request->type == 'monthly' ? $plan->monthly_price : $plan->annual_price);

        $reference = $freshPay->generateReference('plan', $plan->id);
        $nextPayDate = ($plan->package == 'lifetime')
            ? null
            : ($request->type == 'monthly' ? now()->addMonth()->format('Y-m-d') : now()->addYear()->format('Y-m-d'));

        $subscription = new GlobalSubscription();
        $subscription->company_id = $company->id;
        $subscription->package_id = $plan->id;
        $subscription->currency_id = $plan->currency_id ?: global_setting()->currency_id;
        $subscription->package_type = $request->type;
        $subscription->quantity = 1;
        $subscription->gateway_name = 'freshpay';
        ...
        $invoice->package_type = $request->type;
        $invoice->total = $amount;
        $invoice->amount = $amount;
        $invoice->pay_date = now()->format('Y-m-d');
        $invoice->next_pay_date = $nextPayDate;
        $invoice->gateway_name = 'freshpay';
        $invoice->status = 'inactive';
        $invoice->transaction_id = $reference;
        $invoice->event_id = $reference;
        $invoice->token = 'pending';
        $invoice->save();

        $response = $freshPay->initiate($credentials, [
            'merchant_id' => $freshPay->merchantId($credentials),
            'merchant_secrete' => $freshPay->merchantSecret($credentials),
            'amount' => (string) $amount,
            'currency' => $plan->currency?->currency_code ?: 'CDF',
            'action' => 'debit',
            'customer_number' => $freshPay->normalizePhone($request->phone),
            'firstname' => $freshPay->configuredFirstName($credentials, 'ZAA'),
            'lastname' => $freshPay->configuredLastName($credentials, 'ZAA'),
            'email' => $freshPay->configuredEmail($credentials, 'kasisrael@gmail.com'),
            'reference' => $reference,
            'method' => $request->operator,
            'callback_url' => route('billing.save_freshpay-webhook', [global_setting()->hash]),
        ]);

        $body = $response['body'] ?? [];
        $invoice->transaction_id = $body['Transaction_id'] ?? $invoice->transaction_id;
        $invoice->save();
        $subscription->transaction_id = $body['Transaction_id'] ?? $subscription->transaction_id;
        $subscription->save();

        if (!$response['http_ok'] || ($body['Status'] ?? null) !== 'Success') {
            $invoice->token = 'failed';
            $invoice->save();

            Log::warning('FreshPay billing initiate failed', [
            ...
            return Reply::error($body['Comment'] ?? __('messages.paymentFailed'));
        }

        return Reply::successWithData(__('modules.freshpay.requestSubmitted'), [
            'poll_url' => route('billing.freshpay-status', [$invoice->id]),
            'message' => __('modules.freshpay.pendingState'),
            'reference' => $reference,
            'debug' => [
                'stage' => 'initiate',
                'mode' => $credentials->freshpay_mode,
                'endpoint' => $freshPay->endpoint($credentials),
                'merchant_id' => $freshPay->merchantId($credentials),
                'callback_processed' => false,
                'gateway_status' => $body['Status'] ?? null,
                'transaction_status' => $body['Trans_Status'] ?? null,
                'comment' => $body['Comment'] ?? null,
            ],
        ]);
    }

    public function freshpayStatus($invoiceId)
    {
        $invoice = GlobalInvoice::where('company_id', company()->id)->findOrFail($invoiceId);

        if (!in_array($invoice->token, ['complete', 'failed'], true)) {
            $credentials = GlobalPaymentGatewayCredentials::first();
            $freshPay = app(FreshPayService::class);
            $verifyReference = $invoice->event_id ?: $invoice->transaction_id;

            if ($credentials && filled($verifyReference)) {
                $verification = $freshPay->verify($credentials, $verifyReference);
                $data = $verification['body'] ?? [];

                if (!empty($data)) {
                    $this->syncFreshPayBillingInvoice($invoice, $data);
                    $invoice->refresh();
                }
            }
        }
        ...
        return response()->json([
            'status' => 'success',
            'payment_status' => $invoice->token === 'complete' ? 'complete' : ($invoice->token === 'failed' ? 'failed' : 'pending'),
            'message' => $invoice->token === 'complete'
                ? __('superadmin.paymentSuccessfullyDone', ['package' => optional($invoice->package)->name, 'planType' => $invoice->package_type])
                : ($invoice->token === 'failed' ? __('messages.paymentFailed') : __('modules.freshpay.pendingState')),
            'is_final' => in_array($invoice->token, ['complete', 'failed'], true),
            'reload' => $invoice->token === 'complete',
            'source' => in_array($invoice->token, ['complete', 'failed'], true) ? 'verify_or_callback' : 'polling',
            'callback_processed' => in_array($invoice->token, ['complete', 'failed'], true),
            'reference' => $invoice->event_id,
            'transaction_id' => $invoice->transaction_id,
        ]);
    }

    public function freshpayWebhook(Request $request, $hash)
    {
        abort_unless($hash == global_setting()->hash, 404);

        $credentials = GlobalPaymentGatewayCredentials::first();
        $freshPay = app(FreshPayService::class);
        $encryptedPayload = (string) $request->input('data', '');

        Log::info('FreshPay billing webhook received', [
            'signature_present' => filled($request->header('X-Signature')),
            'payload_length' => strlen($encryptedPayload),
        ]);

        if (!$freshPay->verifySignature($request->header('X-Signature'), $encryptedPayload, $credentials->freshpay_hmac_key)) {
            Log::warning('FreshPay billing webhook invalid signature');
            return response()->json(['message' => 'Invalid signature'], 401);
        }

        $payload = $freshPay->decryptPayload($encryptedPayload, $credentials->freshpay_encryption_key);

        if (empty($payload)) {
            Log::warning('FreshPay billing webhook invalid encryption');
            return response()->json(['message' => 'Invalid encryption'], 400);
        }

        Log::info('FreshPay billing webhook decrypted', ['payload' => $payload]);

        $reference = $payload['Reference'] ?? $payload['reference'] ?? null;
        $transactionId = $payload['Transaction_id'] ?? $payload['transaction_id'] ?? null;

        $invoice = GlobalInvoice::where('gateway_name', 'freshpay')
        ...
        if (!$invoice) {
            Log::warning('FreshPay billing webhook invoice not found', [
                'reference' => $reference,
        ...
        $verification = $freshPay->verify($credentials, $invoice->event_id ?: $reference ?: $invoice->transaction_id);
        $data = !empty($verification['body']) ? $verification['body'] : $payload;
        $this->syncFreshPayBillingInvoice($invoice, $data);

        Log::info('FreshPay billing webhook synced invoice', [
            'invoice_id' => $invoice->id,
            'reference' => $invoice->event_id,
            'transaction_id' => $invoice->transaction_id,
            'status' => $invoice->token,
        ]);
    ...
    private function syncFreshPayBillingInvoice(GlobalInvoice $invoice, array $data): void
    {
        $freshPay = app(FreshPayService::class);
        $localStatus = $freshPay->localStatus($data['Status'] ?? null, $data['Trans_Status'] ?? null);

        if ($localStatus === 'complete') {
            $subscription = GlobalSubscription::find($invoice->global_subscription_id);
            $company = Company::withoutGlobalScope(CompanyScope::class)->findOrFail($invoice->company_id);
            $activeSubscription = GlobalSubscription::where('company_id', $company->id)
            ...
            if ($activeSubscription?->gateway_name === 'razorpay') {
                $this->cancelSubscriptionRazorpay();
            }
            ...
            if ($subscription) {
                $subscription->subscription_status = 'active';
                $subscription->transaction_id = $data['Transaction_id'] ?? $subscription->transaction_id;
                $subscription->save();
            }

            $invoice->status = 'active';
            $invoice->transaction_id = $data['Transaction_id'] ?? $invoice->transaction_id;
            $invoice->token = 'complete';
            $invoice->save();

            $package = Package::find($invoice->package_id);
            $company->package_id = $invoice->package_id;
            $company->package_type = $invoice->package_type;
            $company->status = 'active';
            $company->licence_expire_on = ($package && $package->package == 'lifetime') ? null : $invoice->next_pay_date;
            $company->license_updated_at = $invoice->next_pay_date;
            $company->save();

            $generatedBy = User::allSuperAdmin();
            $allAdmins = User::allAdmins($company->id);
            Notification::send($generatedBy, new CompanyUpdatedPlan($company, $invoice->package_id));
            Notification::send($allAdmins, new CompanyUpdatedPlan($company, $invoice->package_id));
