<style>
    ...
    .freshpayPaymentForm{display: none; transition: 0.3s;}
    .freshpayPaymentForm.show{display: block;}
...
<div class="modal-body">
    <div class="form-body">
        @if(!$free)
                ...
                <div class="col-12 col-sm-12 mt-40" id="onlineBox">
                    @if(($paymentGatewatActive))
                        <div class="payment-type">
                            ...
                            @if($stripeSettings->freshpay_status == 'active')
                                <button class="btn-light border rounded f-15 btn px-4 py-3 freshpayPay" type="button">
                                    FreshPay
                                </button>
                            @endif
            ...
            <div class="freshpayPaymentForm">
                @if($stripeSettings->freshpay_status == 'active')
                    <form id="freshpay-billing-form">
                        @csrf
                        <input type="hidden" name="plan_id" value="{{ $package->id }}">
                        <input type="hidden" name="type" value="{{ $type }}">
                        <div class="form-body">
                            <div class="row">
                                <div class="col-lg-12">
                                    <x-forms.text :fieldLabel="__('app.amount')" fieldName="display_amount" fieldId="freshpay_amount"
                                        :fieldValue="global_currency_format(
                                            $package->package == 'lifetime' ? $package->price : ($type == 'monthly' ? $package->monthly_price : $package->annual_price),
                                            $package->currency_id ?: global_setting()->currency_id
                                        )"
                                        :fieldRequired="false" fieldReadOnly="true"/>
                                </div>
                                <div class="col-lg-12">
                                    <x-forms.tel :fieldLabel="__('modules.freshpay.phone')" fieldName="phone" fieldId="freshpay_phone"
                                        :fieldPlaceholder="__('modules.freshpay.phonePlaceholder')" fieldRequired="true"/>
                                </div>
                                <div class="col-lg-12">
                                    <x-forms.select fieldId="freshpay_operator" :fieldLabel="__('modules.freshpay.operator')" fieldName="operator" fieldRequired="true">
                                ...
                                <div class="col-lg-12">
                                    <div id="freshpayBillingStatus" class="alert alert-warning d-none mb-0"></div>
                                </div>
                            </div>
                        </div>
                    </form>
    ...
    @if($stripeSettings->authorize_status == 'active')
        ...
        @if(count($methods) > 0 && !$free)
            <div id="offlineButton">
                ...
                @endphp
                <x-forms.button-primary type="button" class="{{ $class }}" id="save-offline" onclick="selectOffline('{{ $package->id }}')">@lang('app.select')</x-forms.button-primary>
            </div>
        @endif
        @if($free)
        ...
        @endif
        @if($stripeSettings->freshpay_status == 'active' && !$free)
            <div id="freshpayButton">
                <x-forms.button-primary id="save-freshpay-billing-detail" class="d-none">@lang('app.send') <i
                        class="fa fa-arrow-right pl-1"></i></x-forms.button-primary>
            </div>
        @endif
</div>
...
@if(!$free)
    ...
    <script>
        ...
        $('.freshpayPay').click(function (e) {
            e.preventDefault();
            $('.paymentMethods').removeClass('show');
            $('.freshpayPaymentForm').addClass('show');
            $('#freshpayButton #save-freshpay-billing-detail').removeClass('d-none');
            $('.modal-title').text('FreshPay');
        });
        ...
        $('#save-freshpay-billing-detail').on('click', function () {
            $.easyAjax({
                url: '{{ route('billing.freshpay') }}',
                type: 'POST',
                blockUI: true,
                disableButton: true,
                buttonSelector: '#save-freshpay-billing-detail',
                data: $('#freshpay-billing-form').serialize(),
                container: '.modal-content',
                success: function (response) {
                    console.log('[FreshPay Billing] Initiate response', response);

                    if (response.status !== 'success') {
                        console.error('[FreshPay Billing] Initiate failed', response);
                        return;
                    }

                    $('#freshpayBillingStatus').removeClass('d-none').text(response.message);
                    console.log('[FreshPay Billing] Polling started', {
                        poll_url: response.poll_url,
                        reference: response.reference
                    });

                    const poll = setInterval(function () {
                        $.get(response.poll_url, function (statusResponse) {
                            console.log('[FreshPay Billing] Polling response', statusResponse);
                            $('#freshpayBillingStatus').text(statusResponse.message);

                            if (!statusResponse.is_final) {
                                return;
                            }

                            console.log('[FreshPay Billing] Final status received', {
                                payment_status: statusResponse.payment_status,
                                callback_processed: statusResponse.callback_processed,
                ...
                error: function (xhr) {
                    console.error('[FreshPay Billing] HTTP error during initiate', {
                        status: xhr.status,
