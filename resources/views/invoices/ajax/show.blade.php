...
<div class="card border-0 invoice">
    ...
    <div class="card-footer bg-white border-0 d-flex justify-content-start py-0 py-lg-4 py-md-4 mb-4 mb-lg-3 mb-md-3 ">
        ...
        <div class="d-flex">
            ...
            @if (in_array('client', user_roles()) &&
                ...
                <div class="inv-action payNowButton mr-3 mr-lg-3 mr-md-3 dropup">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton"
                        ...
                        @endif
                        @if ($credentials->freshpay_status == 'active')
                            <li>
                                <a class="dropdown-item f-14 text-dark" href="javascript:void(0);"
                                   data-invoice-id="{{ $invoice->id }}" id="freshpayModal">FreshPay</a>
                            </li>
                        @endif
            ...
            @php
                $freshPayPendingPayment = \App\Models\Payment::where('invoice_id', $invoice->id)->where('gateway', 'FreshPay')->where('status', 'pending')->latest()->first();
            @endphp
            @if ($freshPayPendingPayment)
                <div class="alert alert-warning mt-3" id="freshpay-pending-banner"
                     data-poll-url="{{ route('freshpay.payment_status', [$freshPayPendingPayment->id, $invoice->company->hash]) }}">
...
<script>
    ...
    $('body').on('click', '#freshpayModal', function () {
        let id = $(this).data('invoice-id');
        let queryString = "?id=" + id + "&type=invoice";
        let url = "{{ route('front.freshpay_modal') }}" + queryString;
    ...
    const freshPayPendingBanner = $('#freshpay-pending-banner');
    if (freshPayPendingBanner.length) {
        console.log('[FreshPay] Existing pending payment detected', {
            poll_url: freshPayPendingBanner.data('poll-url')
        });
        setInterval(function () {
            $.get(freshPayPendingBanner.data('poll-url'), function (response) {
                console.log('[FreshPay] Pending banner polling response', response);
                freshPayPendingBanner.text(response.message);

                if (response.is_final) {
                    console.log('[FreshPay] Final status from pending banner', {
    ...
    const freshPayNotice = sessionStorage.getItem('freshpay_callback_notice');
    if (freshPayNotice) {
        sessionStorage.removeItem('freshpay_callback_notice');

        try {
            const notice = JSON.parse(freshPayNotice);

            if (window.Swal) {
                Swal.fire({
                    icon: notice.is_success ? 'success' : 'error',
                    text: notice.message,
                    confirmButtonText: 'OK'
                });
            }
        } catch (error) {
            console.error('[FreshPay] Unable to parse callback notice', error);
        }
    }
